...
_.extend(express.directory, {
  //From connect/lib/midleware/directory.js - fixed path to /webinterface directory     
  html: function(req, res, files, next, dir, showUp, icons) {
    fs.readFile(__dirname + '/webinterface/directory.html', 'utf8', function(err, str) {
      if (err) return next(err);
      fs.readFile(__dirname + '/webinterface/css/style.css', 'utf8', function(err, style) {
        if (err) return next(err);
...
function load(icon) {
  ...
  return cache[icon] = fs.readFileSync(__dirname + '/webinterface/icons/' + icon, 'base64');
}
...
app.configure(function() {
  ...
  app.use('/webinterface', express.static(__dirname + '/webinterface'));
  ...
  function viewSource(req, res, next) {
    ...
    fs.readFile(__dirname + '/webinterface/sourceview/index.html', 'utf8', function(err, str) {
      if (err) {
        console.log(err)
        return next(err);
      }
      fs.readFile(__dirname + '/webinterface/css/solarized_dark.css', 'utf8', function(err, style) {
  ...
  app.get('/log.html', function(req, res) {
    fs.readFile(__dirname + '/webinterface/log.html', 'utf8', function(err, str) {
      if (err) {
        console.log(err)
        res.send('ERROR: ' + err);
      } else {

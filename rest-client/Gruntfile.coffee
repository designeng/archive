...
module.exports = (grunt) ->
    ...
    grunt.registerTask "default", ["copyLocalConfig", "connect:server", "watch"]

    # compilation
    grunt.registerTask "coffee-compile-app", ["newer:coffee:app"]
    grunt.registerTask "coffee-compile-jasmine", ["newer:coffee:jasmine"]

    grunt.registerTask "copyLocalConfig", "copy local config to destination", () ->
        done = @async()

        environmentConfigFile = "config/local/config.json"
        destinationConfigFile = "app/js/config.json"

        grunt.file.copy environmentConfigFile, destinationConfigFile
